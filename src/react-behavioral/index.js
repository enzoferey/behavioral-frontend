...
const { Provider: ReactProvider, Consumer } = React.createContext({});
...
function block(blockEvent, waitEvent) {
  const name = "block " + blockEvent + ", wait " + waitEvent;
  var bt = function*() {
    yield {
      wait: [waitEvent],
      block: [blockEvent],
...
export const Provider = class extends React.Component {
  constructor(props) {
    ...
    threads.forEach(thread => {
      if (Array.isArray(thread)) {
        this.bp.addBThread(``, thread[0], thread[1].bind(this));
      } else {
  ...
  render() {
    return <ReactProvider value={this.bp}>{this.props.children}</ReactProvider>;
  }
};
...
class ComponentWithThread extends React.Component {
  ...
  componentDidMount() {
    ...
    bp.addBThread("dispatch", priority || 1, thread.bind(this));
    bp.run();
    this.bp = bp;
  }
...
class ComponentPropsWithThread extends React.Component {
  ...
  // updateView = view => this.setState({ view });
  setProps = (...props) => this.mount && this.setState(...props);
  request = event => request.call(this.props.bp, event);
  bSync = event => bSync.call(this.props.bp, event);
  lastEvent = () => this.props.bp.lastEvent;
...
export function connect(thread) {
  return function(props) {
    return (
      <Consumer>
        {bp => <ComponentWithThread {...props} thread={thread} bp={bp} />}
      </Consumer>
    );
  };
}
...
export function withBp(Component) {
  return function(props) {
    return (
      <Consumer>
        {bp => (
          <Component
            {...props}
            request={event => request.call(bp, event)}
            block={(blockEvent, waitEvent) =>
              block.call(bp, blockEvent, waitEvent)
            }
          />
        )}
      </Consumer>
